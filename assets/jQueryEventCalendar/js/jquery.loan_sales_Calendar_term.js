...
;(function( $ ) {
	...
	$.fn.eventCalendar1 = function(options){
		var calendar1 = this;

		if ( options.locales && typeof(options.locales) == 'string' ) {
			$.getJSON(options.locales, function(data) {
				options.locales = $.extend({}, $.fn.eventCalendar1.defaults.locales, data);
				moment.locale(data.locale, options.locales.moment);
				moment.locale(data.locale);

				initEventCalendar(calendar1, options);
			}).error(function() {
				showError("error getting locale json", $(this));
			});
		} else {
			if ( options.locales && options.locales.locale ) {
				options.locales = $.extend({}, $.fn.eventCalendar1.defaults.locales, options.locales);
	...
	function initEventCalendar(that, options) {
		var eventsOpts = $.extend({}, $.fn.eventCalendar1.defaults, options);
		...
		$( "#term_allocation" ).on('change','#month_sel1', function(e){
			e.preventDefault();
			var year = $('#year_sel1').val(),
				month = $(this).val();
			...
			gettotalinfoterm(pie_month, year);
			// get_total_info_term_pie(pie_month, year);

		});

		$( "#term_allocation" ).on('change','#year_sel1', function(e){
			e.preventDefault();
			var year = $(this).val(),
				month = $('#month_sel1').val();
		...
		$( "#term_allocation" ).on('click','#search_btn1', function(e){
			e.preventDefault();
			var year = flags.wrap.attr('data-current-year'),
				month = flags.wrap.attr('data-current-month');
        ...
        $( "#term_allocation" ).on('click','#clear_search1', function(e){
			e.preventDefault();
			$('#search_text1').val('');
			sale_loan_type = '';
			var year = flags.wrap.attr('data-current-year'),
        ...
        $( "#term_allocation" ).on('click','.sale_loan_type1', function(e){
        	e.preventDefault();
        	sale_loan_type = '';
	...
	function getEventsData(flags, eventsOpts, data, limit, year, month, day, direction){
		...
		$('#loan_sales_calendar_term').animate({
			opacity: eventsOpts.moveOpacity,
			...
			events.push('<table id="loanGenerateList_term" class="table table-bordered loanGenerate_list table-striped role-table m-t-10 dataTable no-footer table-hover" data-page-size="10"><thead><tr><th style="display:none;"></th><th>Loan Address</th><th>City</th><th>State</th><th>Zip</th><th>Total Loan Amount</th><th>Interest Rate</th><th>Borrower Funding Date</th><th>Days Elapsed</th><th>Allocation</th><th>Docs</th><th>Loan Status</th><th>Days Elapsed</th><th>Notes</th><th>Line</th><th>Assignment/Allonge</th> <th>Appraisal/DS</th> </tr></thead><tbody>');
			...
			if ( data.length ) {
				...
				$("#term_unsold_loan").html(total_unsold_loans);
				...
				$.each(data, function(key, event) {
									...
									if(search_text === '' || event.title.toLowerCase().indexOf(search_text.toLowerCase()) != -1){
										...
										if(sale_loan_type == ''){
			                                ...
			                                if((event.colorCode != '') && (event.sale_type == 'tobeallocated')){
			                                	to_be_allocated++;
			                                	to_be_allocated_sum = to_be_allocated_sum +  parseFloat(event.total_loan.replace(/[^0-9-.]/g, ''));
			                                }
			                                ...
			                                $("#term_un_allocated_loans").html(to_be_allocated);
			                                $('#term_un_allocated_loans_amt').html('$'+ number_format(to_be_allocated_sum,2));
			                                $("#term_allocated_loans").html(total_unsold_loans - to_be_allocated);
			                                allocated_loans_amt = loan_amt - to_be_allocated_sum;
				                            $('#term_allocated_loans_amt').html('$'+ number_format(allocated_loans_amt,2));
			                                $("#to_be_allocated_term").html(to_be_allocated);
			                                $("#to_10_14_term").html(to_10_14);
			                                $("#to_15_19_term").html(to_15_19);
			                                $("#above_20_term").html(above_20);
					...
					$('#term_unsold_loan_total').html('$'+ number_format(loan_amt,2));
			...
			$('#loanGenerateList_term').DataTable({
		       "columnDefs": [{
	...
	}, '.dropdown .plan_name_term');

	$(document).on({ 
	    click: function(){ 
			var project_id = $(this).closest('tr').find('select.plan_name_term').prop('id'); 
			$('#floating_term').hide(); 
	...
	$(document).on({ 
	    click: function(){ 
		  ...
		  /*if($("#liquidity").val() == null || $("#liquidity").val() == '' || $("#cost_basis").val() == null || $("#cost_basis").val() == '')
		  {	
			return false;
		  }
		  else
		  {*/
	        ...
	        var userid;
	       	$(".loan_chk:checked").each(function(){
	          	var plan = $(this).closest('tr').find('select.plan_name_term').val();
	            var project_id = $(this).closest('tr').find('select.plan_name_term').prop('id');
				data.push({project_id:project_id,plan:plan});
			});
			//if (($("#liquidity").length > 0)){ 
				var liquidity = $("#liquidity").val();
				var cost_basis = $("#cost_basis").val(); 
				var post_data = {cost_basis:cost_basis,liquidity:liquidity,userid:userid,project:data};
				post_data[global_csrf_token_name]= global_csrf_token_value;
				
				$.ajax({
					url: baseURL + "dashboard/loan_sales_dashboard/save_missing_values",
					type: "POST",
					data: post_data,
					dataType : 'json',
			...
			var post_data = {user_name:name,projects:data,userid:userid,loan_type:'term',tab_type:tab_type};
	    	post_data[global_csrf_token_name]= global_csrf_token_value;
			$.ajax({
		        type: "POST",
		        data:post_data,
		        ...
		        success : function(response){
		        	...
		        	// main_td.find('.floating-submit-button').hide();
		          	if(response!=''){
			          	flag = 1;
	                  	...
	                  	if(response.new_tr!=''){
			          		$(".loanSaleCalendarterm tbody").append(response.new_tr);
			          	}
	...
	/*$(document).on({
	    click: function(){
			...
			// var post_data = {user_name:name,projects:JSON.stringify(data)};
			var post_data = {user_name:name,projects:data,userid:userid};
	    	post_data[global_csrf_token_name]= global_csrf_token_value;
			$.ajax({
		        type: "POST",
		        data:post_data,
	...
	// 		var post_plan_data = {project_id:project_id,plan_id:plan_id,docid:docid,parent:parent,userid:userid};
 ...
 //        	var formData = new FormData($("#save_box_closing_docs")[0]);
	...
	//          	//async:false,
	//             data:postData,
	//             url: baseURL+'dashboard/loan_sales_dashboard/send_box_doc',
	//             success:function(data) {
	//             	$('#send_box').html('Send to box');
	//             	if(data.status = true){
	//             	}
	            ...
	            $('#container-35').highcharts({
	                ...
	                legend: {
	                    ...
	                    labelFormatter: function() {
	                        return '<div style="text-align: left; width:130px;float:left;color:'+this.color+'">' + this.name + '</div><div style="width:40px; margin-right:5px; float:left;text-align:right;color:#f00;color:'+this.color+'">' + this.y + '%</div>';
	                    }
	                },
	                series: [{
	                            innerSize: '85%',
