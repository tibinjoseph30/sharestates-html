...
;(function( $ ) {
	...
	$.fn.eventCalendar2 = function(options){
		var calendar1 = this;

		if ( options.locales && typeof(options.locales) == 'string' ) {
			$.getJSON(options.locales, function(data) {
				options.locales = $.extend({}, $.fn.eventCalendar2.defaults.locales, data);
			...
			if ( options.locales && options.locales.locale ) {
				options.locales = $.extend({}, $.fn.eventCalendar2.defaults.locales, options.locales);
	...
	function initEventCalendar(that, options) {
		var eventsOpts = $.extend({}, $.fn.eventCalendar2.defaults, options);
		...
		$( "#sl_allocation" ).on('change','#month_sel2', function(e){
			e.preventDefault();
			var year = $('#year_sel2').val(),
				month = $(this).val();
			...
			$('#header_month3').html('<b>Projects With Loan Sale Date in '+header_month+'</b>');
			...
			get_total_info_sl(pie_month, year);

		});

		$( "#sl_allocation" ).on('change','#year_sel2', function(e){
			e.preventDefault();
			var year = $(this).val(),
				month = $('#month_sel2').val();
		...
		$( "#sl_allocation" ).on('click','#search_btn2', function(e){
			e.preventDefault();
			var year = flags.wrap.attr('data-current-year'),
				month = flags.wrap.attr('data-current-month');
        ...
        $( "#sl_allocation" ).on('click','#clear_search2', function(e){
			e.preventDefault();
			$('#search_text1').val('');
			sale_loan_type = '';
			var year = flags.wrap.attr('data-current-year'),
        ...
        $( "#sl_allocation" ).on('click','.sale_loan_type2', function(e){
        	e.preventDefault();
        	sale_loan_type = '';
	...
	function getEventsData(flags, eventsOpts, data, limit, year, month, day, direction){
		...
		$('#loan_sales_calendar_sl').animate({
			opacity: eventsOpts.moveOpacity,
			...
			if ( data.length ) {
				...
				$("#sl_unsold_loan").html(total_unsold_loans);
				...
				$.each(data, function(key, event) {
								...
								if (month === false && eventDate1 < new Date()) {
									...
									if(search_text === '' || event.title.toLowerCase().indexOf(search_text.toLowerCase()) != -1){
										if(sale_loan_type == ''){
			                                ...
			                                $("#sl_un_allocated_loans").html(to_be_allocated);
			                                $('#sl_un_allocated_loans_amt').html('$'+ number_format(to_be_allocated_sum,2));
			                                $("#sl_allocated_loans").html(total_unsold_loans - to_be_allocated);
			                                allocated_loans_amt = loan_amt - to_be_allocated_sum;
				                            $('#sl_allocated_loans_amt').html('$'+ number_format(allocated_loans_amt,2));
			                                $("#to_be_allocated_sl").html(to_be_allocated);
			                                $("#to_10_14_sl").html(to_10_14);
			                                $("#to_15_19_sl").html(to_15_19);
			                                $("#above_20_sl").html(above_20);
					...
					$('#sl_unsold_loan_total').html('$'+ number_format(loan_amt,2));
	    ...
	    click: function(){
	        ...
	        var userid;
	       	...
	       	$(".loan_chk:checked").each(function(){
	          	var plan = $(this).closest('tr').find('select.plan_name_sl').val();
	            var project_id = $(this).closest('tr').find('select.plan_name_sl').prop('id');
	    ...
	    click: function(e){
	        ...
	        $.ajax({
	            ...
	            success:function(data) {
	            	...
	            	// 	$("#box_heading").html('<span><img src="http://localhost/sharestates/admin_template/assets/images/loan-closing/icon-buildings.png" class="mr-2"></span>'+ModalLabel);
	            	// } else {
	...
	function get_total_info_sl(month, year){
		var id = $(this).data('id');
