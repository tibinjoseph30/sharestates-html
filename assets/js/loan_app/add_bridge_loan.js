...
$(document).ready(function(){
	...
	/*checking for cookie which loan program for checked*/
	var loan_pgm_chk = readCookie('pipeline_loan_pgm');
	//alert(loan_pgm_chk);
	if(loan_pgm_chk){
		$('input[name="loan_program"][value="'+loan_pgm_chk+'"]').trigger('click');
	}

	$(document).on('ifChecked','.check',function(){
		//alert('d');
		$(this).parent().parent().parent().parent().parent().children('label.error').hide();
		$(this).parent().parent().parent().parent().parent().parent().parent().children('label.error').hide();
	...
	if($('#purchase').prop('checked') == true){
		tr_type = 'P';
	}else if($('#refinance').prop('checked') == true){
		tr_type = 'R';
	}else{
		tr_type = 'RA';
	}
    ...
    $(document).on('ifChecked','.transaction_type',function(){
    	...
    	if($(this).prop('checked') == true){
			var tr_type  = $(this).val();
			if(tr_type == 'R' || tr_type == 'RA'){
				$("#refin-div").show();
				$(".short-sale-div").hide();
				$("#acquisition_loan_request").prev('.control-label').html('Refinance Loan Request <span class="text-danger">*</span>');
				$(".refin-const").show();
				$('input[name="shortsale_approved"]').prop('checked',false).iCheck('update');
				$('.shor-sale-chk').prop('checked',false).iCheck('update');
				$(".affilate-type-chk").prop('checked',false).iCheck('update');
				$(".mem-des-div").hide();
				$(".assignor-chk").prop('checked',false).iCheck('update');
				$("#assignor").val('');
				...
				$("#acquisition_loan_request").prev('.control-label').html('Acquisition Loan Request <span class="text-danger">*</span>');
				$("#refin-div").hide();
				$(".short-sale-div").show();
				$(".refin-const").hide();
				$(".mortgae-chk").prop('checked',false).iCheck('update');
				$("#current_mortgage").hide();
				...
				$(".refin-spent").hide();
				$("#spent_amount").val('');
				$(".paid-receipt").prop('checked',false).iCheck('update');
	...
	/*loan purpose change event*/
	$(document).on('ifChecked','.loan_purpose',function(){
		if($(this).prop('checked') == true){
			...
			if(loan_purpose_type == 'C' || loan_purpose_type == 'B'){
				...
				$("#rehab-div").hide();
				$("#rehab-div2").hide();
				$(".rehab-check").prop('checked',false).iCheck('update');
				$(".rehab-begun").prop('checked',false).iCheck('update');
				if(tr_type == 'P'){
					$("#construction_budget" ).rules("add",{required:true,messages:{required:'Construction budget is required'}});
					$("#construction_budget" ).val('').prop('disabled',false);	
				...
				$("#rehab_budget_completed").val('');
				$('.total_loan').trigger("keyup");				
			}
		}
	});

	/*Rehab Component click event*/
	$(document).on('ifChecked','.rehab-check',function(){
		if($(this).prop('checked') == true){
	...
	/*The Rehab On This Project Begun */
	$(document).on('ifChecked','.rehab-begun',function(){
		if($(this).prop('checked') == true){
	...
	/*Is there a Rehab component */
	$(document).on('ifChecked','#rehab_component_no',function(){
		if($('#purchase').prop('checked') == true){
			$("#construction_budget" ).rules("remove" );
			$("#construction_budget" ).val('').prop('disabled',true);
		}else if(($('#refinance').prop('checked') == true) || ($('#refinance-CEMA').prop('checked') == true)){
	...
	$(document).on('ifChecked','#rehab_component_yes',function(){
	...
	/*Do you anticipate needing more than a 12 month term for yourproject?*/ 
	$(document).on('ifChecked','.more_term',function(){

		if($(this).prop('checked') == true){
			var more_term  = $(this).val();
			if(more_term == 'Y'){
				$("#time-need").show();
			}else{
				$("#time-need").hide();
				$(".loan_term").prop('checked',false).iCheck('update');;
			}
		}
	});

	/*T.O.E. transaction click*/ 
	...
	/*Are you in anyway affiliated with the Assignor*/
	$(document).on('ifChecked','.shor-sale-chk',function(){
	...
	/*Is there an assignment, flip or sale of membership units*/ 
	$(document).on('ifChecked','.affilate-type-chk',function(){
		
		if($(this).prop('checked') == true){
			var affilate_value  = $(this).val();
			if(affilate_value == 'Y'){
				$(".mem-des-div").show();
			}else{
				$(".mem-des-div").hide();
	...
	/*Are you in anyway affiliated with the Assignor*/
	$(document).on('ifChecked','.assignor-chk',function(){
		
		if($(this).prop('checked') == true){
			var assignor_value  = $(this).val();
			if(assignor_value == 'Y'){
				$("#affi-ass-div").show();
			}else{
				$("#affi-ass-div").hide();
	...
	/*Are you in anyway affiliated with the Assignor*/
	$(document).on('ifChecked','.llc-chk',function(){
		
		if($(this).prop('checked') == true){
			var llc_value  = $(this).val();
			if(llc_value == 'Y'){
				$("#entity-div").show();
			}else{
				$("#entity-div").hide();
	...
	/*Do you currently have an existing mortgage on this property?*/
	$(document).on('ifChecked','.mortgae-chk',function(){
		
		if($(this).prop('checked') == true){
			var mortgae_value  = $(this).val();
			if(mortgae_value == 'Y'){
	...
	/*Have you already started Construction/Rehab on the property?*/
	...
	/*construction budget calculation"*/
	...
	$("#transaction-form").validate({
		...
		rules: {
			...
			contract_assigned:{required:'#assignment_flip_type_yes:checked'},
			assignor:{required:'#assignment_flip_type_yes:checked'},
			assignor_affiliate:{required:'#assignment_flip_type_yes:checked'},
			affiliate_description:{required:"#assignor_affiliate_yes:checked"},
		...
		errorPlacement: function(error, element) {
			...
			}else if(element.attr("name") == "shareline_user"|| element.attr("name") == "desired_funding_date" || element.attr("name") == "toe_closing_date" || element.attr("name") == "entity_type" || element.attr("name") == "sales_person" || element.attr("name") == "original_purchase_date"){
				$(element).closest('.form-group').append(error);
			}
	...
	$("#property-info-form").validate({
		...
		rules: {
			...
			residential_units:{required:"#asset_type-residential:checked",min:1,max:4},
			multi_family_units:{required:"#asset_type-multifamily:checked",min:5},
			mixed_use_commercial_units:{required:"#asset_type-mixeduse:checked",min:1},
			mixed_use_residential_units:{required:"#asset_type-mixeduse:checked",min:1},
			commercial_units:{required:"#asset_type-commercial:checked",min:1},
			portfolio_units:{required:"#asset_type-portfolio:checked",min:1},
			current_occupancy:{required:true},
			partial_percentage:{required:"#current_occupancy-partially-occupied:checked"},
		...
		messages: {
			...
			'cl[0][property_addres]':{required:"Property Address is required"},
			'cl[0][city]':{required:"Please select a State"},
			'cl[0][state]':{required:"City is required"},
			'cl[0][zipcode]':{required:"Zip Code is required",minlength:"Please Enter a valid 5-digit Zip code"},
			'cl[0][asset_type]':{required:"Please select a Asset Type"},
			'cl[0][occupancy]':{required:"Please select a Occupancy"},
			'cl[0][phase]':{required:"Please select a Development Phase"},
			'cl[0][as_is_property_value]':{required:"As Is Property Value is required"},
			'cl[0][is_this_property]':{required:"Please select a value"},
			'cl[0][loan_amount]':{required:"Loan Amount is required"},
			'cl[0][loan_being]':{required:"Please select a value"},
		},
		errorPlacement: function(error, element) {
			...
			}else if(element.hasClass('selectpicker') || element.parent().hasClass('input-group') || element.attr("name")=='partial_percentage'){
				$(element).closest('.form-group').append(error);
			}else if(element.hasClass('unit') || element.hasClass('nw-unit')){
		...
		if(asset_type_val == 'Portfolio'){
			...
			if(port_flag == true){
				if($("#additional_collateral_yes").prop('checked') == true){
					...
					$('#more_collatral').find('input:radio').prop('checked',false).iCheck('update');
					$('#more_collatral').find('select').val('').selectpicker('refresh');
					$(".collateral:gt(0)").remove();
					$('#more_collatral').find('.col_com_phase1').hide();
					$('#more_collatral').find('.col_com_dev_phase1').hide();
					$('#more_collatral').find('.show-loan1').hide();
				}
			}
		}
	});
	...
	$(document).on('ifChecked','.occupancy-chk',function(){
		...
		$(".dev-phase").closest('.dev-condition').hide();
		$(".app-plan").prop('checked',false).iCheck('update');
		$("#app-plan-div").hide();
		$("#app-plan-date-div").hide();
		if(ocup_val == 'Vacant'){
			$(".dev-phase").closest('.dev-condition').show();
			$(".dev-phase[value='Leasing']").closest('.dev-condition').hide();
			$(".dev-phase[value='Stabilized']").closest('.dev-condition').hide();
		}else if(ocup_val == 'Partially Occupied'){
			var per  = $('#partial_percentage').val();
	        per      = parseFloat(per.replace(/[^0-9-.]/g, ''));
	        if(per<80){
				$(".dev-phase").closest('.dev-condition').show();
				$(".dev-phase[value='Ground Up Construction']").closest('.dev-condition').hide();
			}else if(per>=80){
				$(".dev-phase[value='In Contract to Sell']").closest('.dev-condition').show();
				$(".dev-phase[value='Rehab Complete & Listed for Sale']").closest('.dev-condition').show();
				$(".dev-phase[value='Leasing']").closest('.dev-condition').show();
			}else{
	        	$(".dev-phase").closest('.dev-condition').show();
	        	$(".dev-phase[value='Ground Up Construction']").closest('.dev-condition').hide();
	        	$(".dev-phase[value='Entitlements']").closest('.dev-condition').hide();
	        }
	...
	/*Development phase */
	$(document).on('ifChecked','.dev-phase',function(){
		if($(this).prop('checked') == true){
			...
			if(dev_phase == 'Conversion' || dev_phase == 'Conversion & Extension/Addition' || dev_phase == 'Ground Up Construction'){
				...
				$(".new-asset-type-div").hide();
				$(".new-asset-chk").prop('checked',false).iCheck('update');;
			}
			...
			if(dev_phase == 'Full Renovation' || dev_phase == 'Full Renovation and Extension' || dev_phase == 'Conversion' || dev_phase == 'Conversion & Extension/Addition' || dev_phase == 'Ground Up Construction'){
				$(".app-plan-div").show();
				if($('#approved_plan_no').prop('checked') == true ){
	...
	$(document).on('ifChecked','.app-plan',function(){
		if($(this).prop('checked') == true){
	...
	/**/
	$(document).on('ifChecked','.new-asset-chk',function(){
		...
		$.get(base_url+'quick_loan_application/get_property_use',{'asset_type':$(this).val()},function(data){
	    	...
	    	if(response.status == true){
			    ...
			    $("#new_property_use").selectpicker('refresh');
			}

		});
	});

	/*----------------------------------------------------------------Additional collaterl section -------------------------------*/
	$(document).on('ifChecked','.add-collateral-chk',function(){
		if($(this).prop('checked') == true){
			var addition_cheeck = $(this).val()
			if(addition_cheeck == 'Y'){
				$("#more_collatral").show();
				$(".add-more").show();
				port_flag = true;
			}else{
	...
	$(document).on('change','.coll_asset_type',function(){
        ...
        //var closestuse= $(this).closest('.prevdiv').next('div').find('select .col_as_is_property_use ');
        //var closestuse= $(this).closest('.prevdiv').next('div').find('select .col_as_is_property_use ');
        var closestuse= $(this).closest('.prevdiv').next('div').find('#col_as_is_property_use'+col_attr);
        console.log(closestuse);
		...
		$.get(base_url+'quick_loan_application/get_property_use',{'asset_type':$(this).val()},function(data){
			...
			if(response.status == true){
				...
				var opt_html = null;
				opt_html = '<option selected value="">--Select--</option>';
				$.each(props,function(key,name){

			       //closestuse.append('<option value="' + name + '">' + name + '</option>');
			       opt_html += '<option value="' + name + '">' + name + '</option>';
			    });
			    closestuse.append(opt_html);
	...
	$(document).on('change','.phase1',function(){
		//alert('d');
		var dev_phase = $(this).val();
	...
	$(document).on('change','.coll_com_asset_type',function(){
		var col_as_attr = $(this).attr('data-id');
		//alert(col_as_attr);

		var closestuse= $(this).closest('.prevdiv').next('div').find('#col_as_com_property_use'+col_as_attr);
        if($(this).val()!=""){
		$.get(base_url+'quick_loan_application/get_property_use',{'asset_type':$(this).val()},function(data){
			var response = $.parseJSON(data);
			var props =  response.prop_use;
	...
	$(document).on('click','.add-more',function(){
		...
		//if(more_id <=3){
			...
			//$("#col_as_is_property_unit"+more_id).rules('add',{required: true});
			//$("#col_as_is_property_use"+more_id).rules('add',{required: true});
	...
	$(document).on('click','#ctn-btn2',function(e){
		var val = $('#property-info-form').validate();
	...
	$(document).on('ifChecked','.appraisal-val-check',function(){
		var appraiser_value = $(this).val();
		if(appraiser_value == 'Y'){
			$("#appraisal-valuation-note").show();
		}else{
			$("#appraisal-valuation-note").hide();
		}
	});
        ...
        $.ajax({
            ...
            success:function(res){
            	if(res.exist=='yes' && exists==0 && user_id==''){
		            ...
		            $("#already_exist_modal").find('input').iCheck({checkboxClass: 'icheckbox_minimal-blue',radioClass: 'iradio_minimal-blue'});
            	}else{
            		$("#already_exist").html('');
            	}
            }
...
function add_more_item(more_id){
	...
	html = '';
	$(".add-more").hide();
	html = '<div id="more_collatral'+more_id+'" class="collateral"><hr /> <div class="row"><div class="col-md-12 col-lg-6"><div class="border-rt"><div class="row"><div class="col-md-6 col-sm-6 col-xs-12"><div class="form-group"><label class="control-label opacity-6">Property Address <span class="text-danger">*</span></label><input type="text" class="form-control collateral_address" rel="'+more_id+'" id="property_addres'+more_id+'" name="cl['+more_id+'][property_addres]" /></div></div><div class="col-md-6 col-sm-6 col-xs-12"><div class="form-group"><label class="control-label opacity-6">City <span class="text-danger">*</span></label><input type="text" class="form-control" id="city'+more_id+'" name="cl['+more_id+'][city]" /></div></div></div><div class="row"><div class="col-md-6 col-sm-6 col-xs-12"><div class="form-group"><label class="control-label opacity-6">State <span class="text-danger">*</span></label><select class="selectpicker" name="cl['+more_id+'][state]" id="state'+more_id+'" required dynamyc-select first-option="Select State" select-type="States" option-label="name" option-value="code" data-size="5"><option value="">Select</option></select></div></div><div class="col-md-6 col-sm-6 col-xs-12"><div class="form-group"><label class="control-label opacity-6">Zip Code <span class="text-danger">*</span></label><input type="text" class="form-control" id="zipcode'+more_id+'" name="cl['+more_id+'][zipcode]" maxlength=" 5" onkeypress="return isNumberKey(event);" /></div></div></div><div class="row"><div class="col-md-6 col-sm-6 col-xs-12"><div class="form-group"><label class="control-label opacity-6">Current Occupancy <span class="text-danger">*</span></label><select class="form-control" name="cl['+more_id+'][occupancy]" id="occupancy'+more_id+'"><option value="">Select</option></select></div></div><div class="col-md-6 col-sm-6 col-xs-12"><div class="form-group"><label class="control-label opacity-6">Development Phase <span class="text-danger">*</span></label><select class="form-control phase1" name="cl['+more_id+'][phase]" id="phase'+more_id+'" data-rel="phase'+more_id+'" data-attr="'+more_id+'" data-size="5"><option value="">Select</option></select></div></div></div><div class="row"><div class="col-md-6 col-sm-6 col-xs-12 col_com_phase'+more_id+' col-complete-div prevdiv" style="display: none;" ><div class="form-group"><label class="control-label opacity-6">As Complete Property Type <span class="text-danger">*</span></label><select class="selectpicker coll_com_asset_type" name="cl['+more_id+'][col_as_com_property_type]" id="col_comp_asset_type'+more_id+'" data-id="'+more_id+'" data-size="5"><option value="">--Select--</option></select></div></div><div class="col-md-6 col-sm-6 col-xs-12 col-complete-div col_com_phase'+more_id+'" style="display: none;"><div class="form-group"><label class="control-label opacity-6">As Complete Property Use <span class="text-danger">*</span></label><select class="selectpicker col_as_com_property_use" name="cl['+more_id+'][col_as_com_property_use]" id="col_as_com_property_use'+more_id+'" data-id="'+more_id+'" data-size="5"><option value="">-- Select --</option></select></div></div></div><div class="row"><div class="col-md-6 col-sm-6 col-xs-12 col-complete-div col_com_phase'+more_id+'" style="display: none;" ><div class="form-group"><label class="control-label opacity-6">As Complete Property Units</label><input type="text" class="form-control" id="col_as_com_property_unit'+more_id+'" name="cl['+more_id+'][col_as_com_property_units]"></div></div><div class="col-md-6 col-sm-6 col-xs-12 col-complete-div col_com_phase'+more_id+'" style="display: none;"><div class="form-group"><label class="control-label opacity-6">As Complete Property value <span class="text-danger">*</span></label><input type="text" class="form-control currency-doller" id="as_com_property_value'+more_id+'" name="cl['+more_id+'][col_as_com_property_value]" placeholder="$0.00"></div></div></div><div class="row"><div class="col-md-6 col-sm-6 col-xs-12 col_com_dev_phase'+more_id+'" style="display: none;" ><div class="form-group"><label class="control-label opacity-6">Please Explain <span class="text-danger">*</span></label><input type="text" class="form-control" id="development_phase_other'+more_id+'" name="cl['+more_id+'][development_phase_other]"></div></div></div></div></div><div class="col-md-12 col-lg-6"><div class="row"><div class="col-md-6 col-sm-6 col-xs-12 prevdiv"><div class="form-group"><label class="control-label opacity-6">As Is Property Type <span class="text-danger">*</span></label><select class="selectpicker coll_asset_type" name="cl['+more_id+'][asset_type]" id="asset_type'+more_id+'" data-id="'+more_id+'" data-size="5"><option value="">Select</option></select></div></div><div class="col-md-6 col-sm-6 col-xs-12"><div class="form-group"><label class="control-label opacity-6">As Is Property Use</label><select class="selectpicker col_as_is_property_use" name="cl['+more_id+'][col_as_is_property_use]" id="col_as_is_property_use'+more_id+'" data-size="5"><option value="">-- Select --</option></select></div></div></div><div class="row"><div class="col-md-6 col-sm-6 col-xs-12"><div class="form-group"><label class="control-label opacity-6">As Is Property Units</label><input type="text" class="form-control" id="col_as_is_property_unit'+more_id+'" name="cl['+more_id+'][col_as_is_property_units]"></div></div><div class="col-md-6 col-sm-6 col-xs-12"><div class="form-group"><label class="control-label opacity-6">As is Property value <span class="text-danger">*</span></label><input type="text" class="form-control currency-doller" id="as_is_property_value'+more_id+'" name="cl['+more_id+'][as_is_property_value]" placeholder="$0.00" /></div></div></div><div class="row"><div class="col-md-6 col-sm-6 col-xs-12"><div class="form-group"><label class="control-label opacity-6">Is this Property? <span class="text-danger">*</span></label><ul class="icheck-list p-0 m-0 d-flex flex-column align-items-start h-100"><li class="mb-3 mw-100"><div class="d-flex w-100"><input tabindex="7" type="radio" class="check is-this-prop"  id="owned-free'+more_id+'" name="cl['+more_id+'][is_this_property]" value="Owned" data-id="'+more_id+'" data-radio="iradio_minimal-blue"><label class="m-b-0 " for="owned-free'+more_id+'">Owned Free and Clear</label></div></li><li class="mb-3 mw-100"><div class="d-flex w-100"><input tabindex="7" type="radio" class="check is-this-prop" id="encumbered'+more_id+'" name="cl['+more_id+'][is_this_property]" value="Encumbered" data-id="'+more_id+'" data-radio="iradio_minimal-blue"><label class="m-b-0 " for="encumbered'+more_id+'">Encumbered</label></div></li></div></div><div class="col-md-6 col-sm-6 col-xs-12 show-loan'+more_id+'" style="display:none;"><div class="form-group"><label class="control-label opacity-6">What is the Loan Amount Currently on it? <span class="text-danger">*</span></label><input type="text" class="form-control currency-doller" id="loan-amount'+more_id+'" name="cl['+more_id+'][loan_amount]" placeholder="$0.00" /></div></div></div><div class="row show-loan'+more_id+'" id="show-loan'+more_id+'" style="display:none;"><div class="col-md-12 col-sm-6 col-xs-12"><div class="form-group"><label class="control-label opacity-6">Will our Loan Being? <span class="text-danger">*</span></label><ul class="icheck-list p-0 m-0 d-flex align-items-center h-100"><li><div class="d-flex w-100"><input tabindex="7" type="radio" class="check" id="paying'+more_id+'" name="cl['+more_id+'][loan_being]" value="Paying"  data-radio="iradio_minimal-blue"><label class="m-b-0 " for="paying'+more_id+'">Paying</label></div></li><li><div class="d-flex w-100"><input tabindex="7" type="radio" class="check" id="taking'+more_id+'" name="cl['+more_id+'][loan_being]" value="Taking a 2nd Position" data-radio="iradio_minimal-blue"><label class="m-b-0 " for="taking'+more_id+'">Taking a 2nd Position</label></div></li></ul></div></div></div><div class="row"><div class="col-md-12 col-sm-12 col-xs-12"><div class="form-group"><a href="javascript:void(0)" class=" delete-more text-danger pull-right" style="margin-left:5px" data-id="'+more_id+'">Remove</a><a href="javascript:void(0)" class="add-more pull-right">Add More</a><div class="clearfix"></div></div></div></div></div></div></div>';


	$("#more_collatral").append(html);
	$('body').find('#state' + more_id).html( $("#state1").html()).selectpicker('refersh');
	$('body').find('#asset_type' + more_id).html($("#asset_type1").html()).selectpicker('refersh');
	$('body').find('#occupancy' + more_id).html($("#occupancy1").html()).selectpicker('refersh');;
	$('body').find('#phase' + more_id).html($("#phase1").html()).selectpicker('refersh');;
	$('body').find('#col_comp_asset_type'+ more_id).html($("#asset_type1").html()).selectpicker('refersh');;
	$("#col_as_is_property_use"+more_id).selectpicker('refersh');
	$("#col_as_com_property_use"+more_id).selectpicker('refersh');
