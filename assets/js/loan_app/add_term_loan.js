	...
	function final_proceesing(){
		...
		if(valid_check == true){
			...
			if($("#refinance-CEMA").prop('checked') == true && $('#property_state').val()!=5 ){
	    		...
	    		$.post(base_url+'backend/term_loan/save_term_loan_data',new_form,function(data){
	    			var response = $.parseJSON(data);
	    			if(response.status == true){
...
function loan_pgm_check( checked_state ){
	if(checked_state == true){
		var program = $('input[name=loan_program]:checked').val();
		...
		if(program=='PL'){
			...
			$("#loan-term_ML").hide();
			$("#rate-term-div").show();
			$('input[name="loan_term_request"]').prop('checked',false).iCheck('update');
			$("#rate-term-div").find('input[name="loan_term_request"][value="5"]').prop('checked',true).iCheck('update');
		}else if(program=='RL'){
			$("#loan-term_RL").hide();
			$("#loan-term_PL").hide();
			$("#loan-term_ML").hide();
			$("#rate-term-div").show();
			$('input[name="loan_term_request"]').prop('checked',false).iCheck('update');
			$("#rate-term-div").find('input[name="loan_term_request"][value="3"]').prop('checked',true).iCheck('update');
		}else if(program=='ML'){
			$("#loan-term_ML").show();
			$("#loan-term_PL").hide();
			$("#loan-term_RL").hide();
			$("#rate-term-div").hide();
			$('input[name="loan_term_request"]').prop('checked',false).iCheck('update');
			$("#loan-term_ML").find('input[name="loan_term_request"][value="12"]').prop('checked',true).iCheck('update');
		}
	}
}
