...
$(document).ready(function(){
	...
	$.validator.addMethod("check_email_exist",function(value,element) {
        ...
        if(userid=="" || value!==email_check){
	        $.ajax({
	            type: "POST",
	            async:false,
	            url: base_url + 'backend/inspectioncompany/check_email_exist',
	            data: postData , 
	...
	// save appraiser
	$(document).on('click','#save_inspection',function(e){
		e.preventDefault(e);

        var appl_type = $(this).attr('data-type');
		var inspection_form_valid = $("#add-inspection-form").valid();

		if(inspection_form_valid == true){ 
            $(this).prop('disabled',true);
			var form = $('#add-inspection-form')[0];
            var data = new FormData(form);
            $.ajax({
                url:base_url + "backend/inspectioncompany/add",
                ...
                success:function(result){
                	...
                	if(result.status = true){
                        if(appl_type == 'add')
                           window.location.href = base_url+'backend/inspectioncompany';
                        else 
                            window.location.href = base_url+'backend/inspectioncompany/edit_inspection/'+response.inspection_id;
                	}
                }
            });
		}
    ...
    $(document).on('click','#cancel_inspection',function(e){
        window.location.href = base_url+'backend/inspectioncompany';
    });
