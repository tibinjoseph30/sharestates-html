...
$(document).ready(function(){
    ...
    // save sub attorney
	$(document).on('click','#save_foreclosure_sub',function(e){
		e.preventDefault(e);

		var foreclosure_form_valid = $("#add-foreclosure-attorney-form").valid();

		if(foreclosure_form_valid == true){
			var form = $('#add-foreclosure-attorney-form')[0];
            var data = new FormData(form);
            $.ajax({
                url:base_url + "backend/foreclosure_attorney/add_subaccount",
    ...
    $(document).on('click','#cancel_foreclosure_sub',function(e){
        window.location.href = base_url+'backend/foreclosure_attorney';
    });
});