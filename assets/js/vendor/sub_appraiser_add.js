...
$(document).ready(function(){
    ...
    $("#add-sub_appraiser-form").validate({
        onkeyup: false,
        ignore : false,
        rules: {
    ...
    // save sub attorney
	$(document).on('click','#save_appraiser_sub',function(e){
		e.preventDefault(e);

		var title_form_valid = $("#add-sub_appraiser-form").valid();

		if(title_form_valid == true){
			var form = $('#add-sub_appraiser-form')[0];
            var data = new FormData(form);
            $.ajax({
                url:base_url + "backend/appraiser/add_subaccount",
    ...
    $(document).on('click','#cancel_appraiser_sub',function(e){
        window.location.href = base_url+'backend/appraiser';
    });
});