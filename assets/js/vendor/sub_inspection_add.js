...
$(document).ready(function(){
    ...
    $("#add-sub_inspection-form").validate({
        onkeyup: false,
        ignore : false,
        rules: {
    ...
    // save sub attorney
	$(document).on('click','#save_inspection_sub',function(e){
		e.preventDefault(e);

		var inspection_form_valid = $("#add-sub_inspection-form").valid();

		if(inspection_form_valid == true){
			var form = $('#add-sub_inspection-form')[0];
            var data = new FormData(form);
            $.ajax({
                url:base_url + "backend/inspectioncompany/add_subaccount",
    ...
    $(document).on('click','#cancel_inspection_sub',function(e){
        window.location.href = base_url+'backend/inspectioncompany';
    });
});