...
$(document).ready(function(){
	...
	$.validator.addMethod("check_email_exist",function(value,element) {
        ...
        if(userid=="" || value!==email_check){
	        $.ajax({
	            type: "POST",
	            async:false,
	            url: base_url + 'backend/appraiser/check_email_exist',
	            data: postData , 
	...
	$(document).on('click','#save_appraiser',function(e){
		...
		if(appraiser_form_valid == true && loctn.length != 0){ 
            ...
            $.ajax({
                url:base_url + "backend/appraiser/add",
                ...
                success:function(result){
                	...
                	if(response.status = true){
                        if(appl_type == 'add')
                    	   window.location.href = base_url+'backend/appraiser';
                        else 
                            window.location.href = base_url+'backend/appraiser/edit_appraiser/'+response.appraiser_id;
                	}
                }
            });
		}
    ...
    $('body').on('click', '.delete-doc', function(){  
        ...
        if(project_confirm==true){
            $.ajax({
                type: 'POST',
                data: post_data,
                url:  base_url + 'backend/appraiser/appraiser_delete_doc',
                dataType: "html",
    ...
    var data_property = {};
    var url_chart_property   = base_url + 'backend/appraiser/loan_chart_property';
    ...
    var data_location= {};
    var url_chart_location   = base_url + 'backend/appraiser/loan_chart_location';
    ...
    $(document).on('click','#unblock-user',function(){
        var appr_id = $(this).attr('data-id');
        var type = $(this).attr('data-type');
        status_update_refresh({'status':type,'appraiser_id':appr_id,'csrf_test_name':global_csrf_token_value},base_url+'backend/appraiser/change_block_status',"Do you really want to unblock this appraiser?","Appraiser  Unblocked Successfully.","Appraiser could not be unblocked",'appraiser-table-list');
    ...
    $(document).on('click','#app-activate',function(){
        var appr_id = $(this).attr('data-id');
        var type = $(this).attr('data-type');
        status_update_refresh({'status':type,'appraiser_id':appr_id,'csrf_test_name':global_csrf_token_value},base_url+'backend/appraiser/change_status',"Do you really want to change status of this appraiser?","Appraiser status updated successfully.","Appraiser status is could not be updated",'appraiser-table-list');
    });
