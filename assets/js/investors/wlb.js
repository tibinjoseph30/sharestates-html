...
$(document).ready(function(){
    ...
    $.ajax({
        type: 'POST',
        data: post_data,
        url:  base_url+'backend/investors/get_wlb_count_amt',
	...
	var myDataTable =  $('#wlbList').DataTable({
        "dom": '<<t>p>',
        "aLengthMenu":false,
        "paging":   true,
        "bLengthChange": true,
        ...
        "ajax": {
            "url": base_url+'backend/investors/ajax_whole_loan_buyers_list',
            "type": "GET",
            "data": function(d){
    ...
    $(document).on('click','.user_activate',function(){
        var user_id = $(this).attr('data-id');
        var type = $(this).attr('data-type');
        status_update({'status':type,'user_id':user_id,'csrf_test_name':global_csrf_token_value},base_url+'backend/investors/change_status',"Do you really want to change status of this investor?","Investor status updated successfully.","Investor status is could not be updated",'wlbList');
    });

    $(document).on('click','.user-delete',function(){
        var user_id = $(this).attr('data-id');
        // var type = $(this).attr('data-type');
        status_update({'user_id':user_id,'csrf_test_name':global_csrf_token_value},base_url+'backend/investors/delete',"Do you really want to delete this user?","User delete successfully.","User could not be deleted",'wlbList');
    });
    ...
    $('#download_accredited').on('click',function(){
        ...
        window.location.href = base_url + "backend/investors/export_wlb?search="+search+"&date_range="+date_range+"&from_date="+from_date+"&to_date="+to_date;
    });
