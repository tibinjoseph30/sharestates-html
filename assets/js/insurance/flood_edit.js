...
$(document).ready(function(){
    ...
    $(document).on('click','#save_flood',function(e){ 
        e.preventDefault();

        var appl_type = $(this).attr('data-type');
        
		var flood_form_valid = $("#add-flood-form").valid();

		if(flood_form_valid == true){ 

            
            $(this).prop('disabled',true);
            
			var form = $('#add-flood-form')[0];
            var data = new FormData(form);
            $.ajax({
                url:base_url + "backend/manage_insurance/add_flood",
                ...
                success:function(result){
                	...
                	if(response.status = true){
                        if(appl_type == 'add')
                    	   window.location.href = base_url+'backend/manage_insurance/flood_insurance';
                        else 
                            window.location.href = base_url+'backend/manage_insurance/edit_flood/'+response.insurance_id;
                	}
                }
            });
		}
    ...
    $(document).on('click','#cancel_flood',function(e){
